    ...
    <Card className="gap-y-4">
      ...
      >
        {fuelTypes.map((type, index) => (
          <Pressable
            onPress={() => handleFuelType(type)}
            key={index}
            className={
              fuel.type === type
                ? "text-center p-2 rounded-xl bg-neutral-300"
                : "text-center p-2 rounded-xl bg-neutral-700"
            }
          >
            <Text className={fuel.type === type ? "text-black" : "text-white"}>
              {type}
            </Text>
          </Pressable>
        ))}
      ...
      <View className="flex-row w-full items-center justify-between">
        ...
        <View className="flex-col gap-1 w-1/2 items-center">
          <Text className="text-neutral-400 font-semibold">Mensual</Text>
